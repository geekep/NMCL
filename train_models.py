...
def main():
	...
	#torch.cuda.set_device('cuda')#在cpu中应该可以删去，指定需要使用的GPU

	# Names, paths, logs
	parser = argparse.ArgumentParser()
	parser.add_argument('--dataset_path', default='D:\抑郁症工作\迎合数据集', help='path to dataset')#修改
	...
	# Data parameters
	parser.add_argument('--feature_dim', default=208, type=int, help='dimensionality of the features (mfcc = 39|eGeMAPS = 23|AUpose = 49|ResNet = 2048')#修改
	parser.add_argument('--feature_type', default='CLNF_features3D', help='mfcc|eGeMAPS|AUpose|ResNet|VGG|DS_densenet')
	parser.add_argument('--workers_num', default=4, type=int, help='number of workers for data loading')#workers_num究竟指的是什么意思呢workers_num=4
	...
	parser.add_argument('--modality', default='vision', help='speech|vision')#调整默认值
	...
	train_data = pd.read_csv(os.path.join(opt.dataset_path, 'train_split_Depression_AVEC2017.csv'), header=0)#train_split.csv
	train_ids = train_data['Participant_ID'].tolist()#'Participant_ID'

	val_data = pd.read_csv(os.path.join(opt.dataset_path, 'dev_split_Depression_AVEC2017.csv'), header=0)#'dev_split.csv'
	val_ids = val_data['Participant_ID'].tolist()#Participant_ID

	test_data = pd.read_csv(os.path.join(opt.dataset_path, 'test_split_Depression_AVEC2017.csv'), header=0)#test_split.csv
	test_ids = test_data['participant_ID'].tolist()#Participant_ID

	ids = {}

	if not os.path.exists(opt.logger_path):
		os.makedirs(opt.logger_path)#创建了logger_path路径的文件夹
	if not os.path.exists(opt.out_path):
		os.makedirs(os.path.join(opt.out_path, 'predictions', 'val'))#在out_path下创建了presictions文件夹，然后在predictions文件夹下创建了val文件夹
	...
	set_logger(os.path.join(opt.logger_path, opt.logger_name))#为什么这里又有一个语句

# Data loaders
	if opt.modality=='speech' or opt.modality=='vision':
		train_loader, val_loader, test_loader = get_loaders_unimodal_regressor_sequence_dataset(ids=ids,opt=opt)#加载数据
	else:
	...
	if opt.modality=='speech' or opt.modality=='vision':
		if opt.model_type=='unimodal-regressor-sequence':
			model = UnimodalRegressorSequence(opt=opt).cpu()#在cpu中只需要将cuda()转化为cpu()
...
def train_unimodal_regressor_sequence(train_loader, model, optimizer, opt):
	...
	for i, train_data in enumerate(train_loader):#enumerate() 函数用于将一个可遍历的数据对象组合为一个索引序列，同时列出数据和数据下标
		features, lengths, labels, _ = train_data#修改features, lengths, labels, _
		#features, lengths, labels, _ = Variable(features),Variable(lengths),Variable(labels)

		optimizer.zero_grad()

		features = features.cpu()#修改
		labels = labels.cpu()#修改
...
def set_logger(log_path):
	...
	if not logger.handlers:#判断handerl列表是否为空，只建立一个handler
