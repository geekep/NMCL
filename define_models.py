...
class UnimodalRegressorSequence(torch.nn.Module):
	...
	def __init__(self, opt):
		...
		self.linear2 = torch.nn.Linear(opt.hidden_layer_dim, self.output_dim)#对传入数据应用线性变换：y = Ax+b
	...
	def forward_once(self, seq, length):
		...
		I = Variable(I.expand(seq.size(0), 1, self.rnn_layer_dim) - 1).cpu()#修改
